...
while(is.null(sc)) {
 ...
 out <- bayesrules::summarize_normal_normal(
  ...
  ## solution
  res <- round(dnorm(mu_post, mu_post, sd_post), 2)
  
  ## schoice
  sc <- num_to_schoice(res, range = c(0.01, 2.0), delta = 0.1, digits = 2)
}
```
...
Si trovi il valore massimo della densità della distribuzione a posteriori $p(\mu \mid y, \sigma)$.
