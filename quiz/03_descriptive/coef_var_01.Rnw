...
## SOLUTION
n <- length(x)
std_dev <- sqrt(var(x) * (n - 1) / n)
sol <- round(std_dev / mean(x), 2)
...
solutions[1] <- TRUE
solutions[2:5] <- FALSE
explanations[1] <- "Si utilizzi la formula per il coefficiente di variazione"
explanations[2:5] <- "Si veda la spiegazione"
@
    
\begin{question}
Si consideri la variabile $X$  = \Sexpr{x[1]}, \Sexpr{x[2]}, \Sexpr{x[3]}, \Sexpr{x[4]}. Si calcoli il coefficiente di variazione di $X$.
