...
Ad un bambino di 8 anni vengono sottoposti 7 problemi. Le sue risposte (C = risposta corretta, S = risposta sbagliata) sono le seguenti: C, C, S, C, S, C, C. Utilizzando una distribuzione a priori Beta di parametri alpha = 20 e beta = 2, si calcoli la moda della distribuzione a posteriori per la probabilità di rispondere correttamente. Si usi il metodo dell'approssimazione numerica.
